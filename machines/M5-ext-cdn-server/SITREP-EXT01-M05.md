...
**Description:**
- Nginx Alias Off-By-One Path Traversal detected on machine M5-ext-cdn-server (203.x.x.x:80)
- Service affected: BUNL CDN Static File Server (Nginx)
- Attack vector: Request to /assets../config.py bypasses webroot restriction; config.py returned by Nginx
- Attacker successfully exploited the vulnerability; evidence found in /var/log/bunl/cdn-server/access.log

**Severity Level:** Critical

**Impact:** config.py disclosed; CORP_PIVOT_TOKEN and CDN credentials exposed

**Affected System:** M5-ext-cdn-server — BUNL CDN Static File Server (Nginx) (Port 80)
...
**Detection Method:**
Review of application logs in /var/log/bunl/cdn-server/access.log reveals unusual request patterns consistent with exploitation. Specific indicators include:

- **IOC:** GET /assets../config.py returning 200 in Nginx access log

To detect:
```bash
grep -i "GET" /var/log/bunl/cdn-server/access.log | tail -50
```

**Initial Detection Time:** [Timestamp when incident was first detected]

**Attack Vector:** Nginx Alias Off-By-One Path Traversal
...
**Containment:**
- Isolate M5-ext-cdn-server from the external network segment
...
**Evidence:**
- Access logs at /var/log/bunl/cdn-server/access.log
- Application configuration files (where applicable)
- Network connection records

**Indicators of Compromise (IOCs):**
- GET /assets../config.py returning 200 in Nginx access log
...
**MITRE ATT&CK: T1083**
Description: Nginx Alias Off-By-One Path Traversal used to gain unauthorised access to BUNL infrastructure services. The attacker exploited an application-layer vulnerability to access privileged configuration data and obtain credentials for lateral movement to the next zone.

**Mitigation Recommendations:**
- Apply secure coding fix for Nginx Alias Off-By-One Path Traversal
